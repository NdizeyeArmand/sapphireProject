...
dotenv.config();
const Config = { ...require('../config/config.json'), client: { ...require('../config/config.json').client, token: process.env.DISCORD_TOKEN } };
...
async function start(): Promise<void> {
    ...
    try {
        if (Config.clustering.enabled) {
            ...
            let requiredShards = await ShardUtils.requiredShardCount(process.env.DISCORD_TOKEN);
