...
export class Bot {
    ...
    private registerListeners(): void {
        ...
        this.client.on(Events.ShardReady, (shardId: number, unavailableGuilds?: Set<string>) =>
            this.onShardReady(shardId, unavailableGuilds ?? new Set<string>())
        );
    ...
    private onShardReady(shardId: number, _unavailableGuilds: Set<string> = new Set<string>()): void {
    ...
    private async onGuildJoin(guild: Guild): Promise<void> {
        ...
        try {
            await this.guildJoinHandler.run(guild);
        } catch (error) {
    ...
    private async onGuildLeave(guild: Guild): Promise<void> {
        ...
        try {
            await this.guildLeaveHandler.run(guild);
        } catch (error) {
    ...
    private async onMessage(msg: Message): Promise<void> {
        ...
        try {
            ...
            await this.messageHandler.run(msg);
        } catch (error) {
    ...
    private async onInteraction(intr: Interaction): Promise<void> {
        ...
        if (intr instanceof CommandInteraction || intr instanceof AutocompleteInteraction) {
            try {
                await this.commandHandler.run(intr);
            } catch (error) {
                Logger.error(Logs.error.command, error);
            }
        } else if (intr instanceof ButtonInteraction) {
            try {
                await this.buttonHandler.run(intr);
            } catch (error) {
    ...
    private async onReaction(
        ...
        if (!msgReaction || !reactor) {
            return; // Early exit if either is undefined
        }
        if (
            !this.ready ||
        ...
        try {
            ...
            await this.reactionHandler.run(msgReaction, reactor);
        } catch (error) {
